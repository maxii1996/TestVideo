 ...
 */
    ...
    class ProgressBar {
        ...
        draw() {
            ...
            context.beginPath();
            if (this.type === 'Full Circle') {
                context.arc(this.posX, this.posY, this.radius, -Math.PI / 2, -Math.PI / 2 + progress * Math.PI * 2);
            } else if (this.type === 'Kingdom Hearts') {
                if (progress <= 0.75) {
                    context.arc(this.posX, this.posY, this.radius, -Math.PI / 2, -Math.PI / 2 + progress * Math.PI * 2);
                } else {
                    context.arc(this.posX, this.posY, this.radius, -Math.PI / 2, Math.PI * 1.5);
                    context.moveTo(this.posX + this.radius * Math.cos(Math.PI * 1.5), this.posY + this.radius * Math.sin(Math.PI * 1.5));
                    context.lineTo(this.posX + this.radius * Math.cos(Math.PI * 1.5) + (progress - 0.75) * 4 * this.radius, this.posY + this.radius * Math.sin(Math.PI * 1.5));
                }
            }
            context.lineWidth = this.lineWidth;
    ...
    PluginManager.registerCommand('ProgressBar', 'showProgressBar', args => {
        const { id, actualValue, minValue, maxValue, posX, posY, radius, lineWidth, color1, color2, color3, backgroundColor, text, textPosX, textPosY, font, fontSize, fontColor, type } = args;
        let sprite = ProgressBars.list[id] ? ProgressBars.list[id].sprite : new Sprite();
        ImageManager.loadSystem('Window').addLoadListener(() => {
            ProgressBars.list[id] = new ProgressBar(id, sprite, actualValue, minValue, maxValue, posX, posY, radius, lineWidth, color1, color2, color3, backgroundColor, text, textPosX, textPosY, font, fontSize, fontColor, type);
        });
    });
