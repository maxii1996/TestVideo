...
class Window_CustomStatList extends Window_SkillBase {
  ...
  drawItemNumber (item, x, y, width) {
    ...
    const boxWidth = this.textWidth('00') + 6;
    const rect = new Rectangle();
    rect.x = x + width - (this.textWidth('00'));
    rect.y = y;
    rect.width = boxWidth;
    rect.height = this.lineHeight();
    ...
    if (statPlus.currentLevel - level !== 0) {
      ...
      this.contents.fontSize = this.contents.fontSize - 16;
...
class Window_Confirmation extends Window_HorzCommand {
  ...
  makeCommandList () {
    this.addCommand('${stats_distribution.si_option}', '${stats_distribution.si_option}');
    this.addCommand('${stats_distribution.no_option}', '${stats_distribution.no_option}');
    this.addCommand('${stats_distribution.cancel_option}', 'cancel');
  }
}

/* globals ColorManager VisuMZ */
...
class Scene_StatLevel extends Scene_MenuBase {
  ...
  createStatWindow () {
    ...
    const statListRect = new Rectangle(
      titleRect.x,
      titleRect.y + titleRect.height + 100,
      titleRect.width,
      400
    );
    const titleWindow = new Window_Title(titleRect);
    titleWindow.setTitle('${stats_distribution.dist_label}');
	titleWindow.y = 150;
  ...
  createStatPointsWindow () {
    const rect = new Rectangle(
      ...
      this._windowStatList.y + this._windowStatList.height + 100,
      this._windowStatList.width,
      60
    );
    this._statPointsWindow = new Window_SkillPoints(rect);
    this._statPointsWindow.setPoints('${stats_distribution.stat_points}', this._actor._customStatPoints);
  ...
  createSkillPointsWindow () {
    const rect = new Rectangle(
      this._windowSkillList.x + 30000,
      this._windowSkillList.y + this._windowSkillList.height + 200,
  ...
  helpWindowRect () {
    const ww = this.UIWidth  - 340;
    const wh = this.UIHeight - (this.buttonAreaHeight() + this._statusWindow.height) + 1;
    const wx = 120;
    const wy = this._statPointsWindow.y + this._statPointsWindow.height + 165;
  ...
  statusWindowRect () {
    ...
    const ww = this._statPointsWindow.width * 2;
    const wh = this._windowSkillList.height + (this._skillPointsWindow.height * 1.45) + 400;
    return new Rectangle(wx, wy, ww, wh)
  }
  ...
  createConfirmationWindow () {
    ...
    this._confirmWindow.x -= this._confirmWindow.width / 2 + 150;
    this._confirmWindow.y -= this._confirmWindow.height / 2;

    this._confirmWindow.setHandler('${stats_distribution.si_option}', this.onConfirmOk.bind(this));
    this._confirmWindow.setHandler('${stats_distribution.no_option}', this.onConfirmNo.bind(this));
    this._confirmWindow.setHandler('cancel', this.onConfirmCancel.bind(this));

    const titleRect = new Rectangle(
      this._confirmWindow.x,
      this._confirmWindow.y - this._confirmWindow.height,
      this._confirmWindow.width + 370,
      70
    );
    this._confirmTitle = new Window_Title(titleRect);
    this._confirmTitle.largeFont = false;
    this._confirmTitle.setTitle('${stats_distribution.save_changes}');
  ...
  onConfirmOk () {
    ...
    this.popScene();
	$gameVariables.setValue(180, this._actor._customStatPoints);
