...
CycloneSteam.start = function() {
  PluginManager.registerCommand('CycloneSteam', 'getScreenName', args => {
    console.log('getScreenName command is running');

    const variableId = Number(args.variableId);
    if (isNaN(variableId) || variableId < 1) {
      console.error('You must provide a valid variable ID.');
      return;
    }

    const screenName = CycloneSteam.screenName;
    console.log(`Screen name from CycloneSteam: ${screenName}`);

    $gameVariables.setValue(variableId, screenName);
    console.log(`Variable ${variableId} after set: ${$gameVariables.value(variableId)}`);
  });
};
